...
const TrackPointStandalone = () => {
  ...
  const AltisConfig = {
    minZoom: 0,
    maxZoom: 6,
    CRS: null, // Will be set when Leaflet loads
  ...
  const initializeMap = () => {
    if (!window.L) {
      debugLog('Leaflet not loaded yet', 'error');
      return;
    }
    ...
    try {
      // Set up custom CRS
      AltisConfig.CRS = window.L.extend({}, window.L.CRS.Simple, {
  ...
  const addMapBorder = () => {
    ...
    try {
      ...
      mapBorderRef.current = window.L.rectangle(bounds, {
        color: '#ff0000',
        weight: 3,
        opacity: 0.8,
        fillOpacity: 0,
  ...
  const loadLeaflet = () => {
    return new Promise((resolve, reject) => {
      ...
      leafletJS.onload = () => {
        debugLog(`Leaflet loaded successfully - version: ${window.L.version}`, 'success');
        resolve();
      };
      leafletJS.onerror = () => {
        debugLog('Failed to load Leaflet', 'error');
  ...
  useEffect(() => {
    debugLog('TrackPoint component mounted - starting initialization', 'info');
    ...
    return () => {
      if (mapInstanceRef.current) {
        ...
        debugLog('Map instance cleaned up', 'info');
      }
    };
  }, []);
  ...
  return (
    <div className="trackpoint-container">
      ...
      <div className="trackpoint-header">
        <h1>TrackPoint - Altis Tactical Map</h1>
        <div className="header-controls">
          <button className="btn" onClick={centerMap}>
            <i className="fas fa-crosshairs"></i> Center
          </button>
          <button 
            className={`btn cities ${citiesVisible ? 'active' : ''}`} 
            onClick={toggleCities}
          >
            <i className="fas fa-city"></i> Cities
          </button>
          <button 
            className={`btn ${borderVisible ? 'active' : ''}`} 
            onClick={toggleBorder}
          >
            <i className="fas fa-border-style"></i> Border
          </button>
          <button 
            className="btn debug" 
...
export default TrackPointStandalone;